...
@NullMarked
public final class NmsAdvancementHandler_v1_20_6 implements INmsAdvancementHandler {
    ...
    static {
        try {
            ...
            throw new RuntimeException("NmsAdvancementHandler_v1_20_6 init failed", e);
        }
    }
    ...
    private static DisplayInfo buildDisplay(AdvancementDisplaySpec spec) {
        ...
        Optional<ResourceLocation> bg = buildBackground(spec.background());

        return new DisplayInfo(icon, title, desc, bg, type,
                spec.showToast(), spec.announceToChat(), spec.hidden());
    }

    private static Optional<ResourceLocation> buildBackground(@Nullable String background) {
        if (background == null) return Optional.empty();
        try {
            ResourceLocation rl = ResourceLocation.tryParse(background);
            if (rl == null) throw new IllegalArgumentException("Invalid resource location: " + background);
    ...
    private static AdvancementRewards buildRewards(AdvancementSpec spec) {
        // Note: Registries.RECIPE does not exist in 1.20.6; recipe rewards are not supported.
        if (spec.rewardExperience() == 0) {
            return AdvancementRewards.EMPTY;
        }
        return new AdvancementRewards(spec.rewardExperience(), List.of(), List.of(), Optional.empty());
    }
        ...
        if (!hiddenAdded.isEmpty()) {
            for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                ...
                ((CraftPlayer) onlinePlayer).getHandle().connection.send(
                        new ClientboundUpdateAdvancementsPacket(false, Collections.emptyList(), toHide, Collections.emptyMap())
                );
            }
        }
    ...
    public boolean award(Player player, NamespacedKey key, String criterionName) {
        ...
        if (holder.value().display().map(DisplayInfo::isHidden).orElse(false)) {
            ((CraftPlayer) player).getHandle().connection.send(
                    new ClientboundUpdateAdvancementsPacket(false, List.of(holder), Collections.emptySet(), Collections.emptyMap())
            );
        }
        PlayerAdvancements pa = ((CraftPlayer) player).getHandle().getAdvancements();
    ...
    public void removeIncompleteHiddenAdvancements(Player player, Collection<NamespacedKey> hiddenKeys) {
        ...
        ((CraftPlayer) player).getHandle().connection.send(
                new ClientboundUpdateAdvancementsPacket(false, Collections.emptyList(), toRemove, Collections.emptyMap())
        );
    }
