...
@NullMarked
public final class NmsAdvancementHandler_v1_21_10 implements INmsAdvancementHandler {
    ...
    static {
        try {
            ...
            throw new RuntimeException("NmsAdvancementHandler_v1_21_10 init failed", e);
        }
    }
    ...
    private static Optional<ClientAsset.ResourceTexture> buildBackground(@Nullable String background) {
        ...
        try {
            return Optional.of(new ClientAsset.ResourceTexture(ResourceLocation.parse(background)));
        } catch (Exception e) {
    ...
    private static void broadcastAdded(List<AdvancementHolder> holders) {
        ClientboundUpdateAdvancementsPacket packet = new ClientboundUpdateAdvancementsPacket(
                false, holders, Collections.emptySet(), Collections.emptyMap(), false
        );
