...
@NullMarked
public final class NmsAdvancementHandler_v1_21_1 implements INmsAdvancementHandler {
    ...
    static {
        try {
            ...
            throw new RuntimeException("NmsAdvancementHandler_v1_21_1 init failed", e);
        }
    }
    ...
    private static Optional<ResourceLocation> buildBackground(@Nullable String background) {
        ...
        try {
            return Optional.of(ResourceLocation.parse(background));
        } catch (Exception e) {
    ...
    private static AdvancementRewards buildRewards(AdvancementSpec spec) {
        ...
        List<ResourceLocation> recipes = spec.rewardRecipes().stream()
                .map(k -> ResourceLocation.fromNamespaceAndPath(k.getNamespace(), k.getKey()))
                .toList();
