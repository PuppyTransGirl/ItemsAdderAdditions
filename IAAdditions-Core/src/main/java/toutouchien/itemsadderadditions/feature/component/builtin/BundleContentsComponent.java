...
@NullMarked
@Component(key = "bundle_contents")
public final class BundleContentsComponent extends ComponentExecutor {
    ...
    public boolean configure(@Nullable Object configData, String namespacedID) {
        if (!(configData instanceof List<?> rawList)) {
            Log.itemWarn("Components", namespacedID, "'bundle_contents' must be a list of item IDs");
            return false;
        }
        ...
        for (Object entry : rawList) {
            ...
            if (item == null) {
                Log.itemWarn("Components", namespacedID, "'bundle_contents' item '{}' not found.", id);
                return false;
            }
            resolved.add(item.clone());
        }
        if (resolved.isEmpty()) {
            Log.itemWarn("Components", namespacedID, "'bundle_contents' list must not be empty");
            return false;
        }
        this.contents = List.copyOf(resolved);
        return true;
    }

    @Override
    public ItemStack apply(ItemStack itemStack, String namespacedID) {
        itemStack.setData(DataComponentTypes.BUNDLE_CONTENTS, BundleContents.bundleContents(contents));
        return itemStack;
    }
}
