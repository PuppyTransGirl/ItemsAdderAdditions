...
@NullMarked
@Component(key = "map_color")
public final class MapColorComponent extends ComponentExecutor {
    private @Nullable Color color;

    @Override
    public boolean configure(@Nullable Object configData, String namespacedID) {
        if (!(configData instanceof String raw)) {
            Log.itemWarn("Components", namespacedID, "'map_color' must be a hex color string (#RRGGBB)");
            return false;
        }

        String hex = raw.trim();
        if (!hex.startsWith("#") || hex.length() != 7) {
            Log.itemWarn("Components", namespacedID, "'map_color' value '{}' is not a valid hex color. Use #RRGGBB.", raw);
            return false;
        }
        ...
        try {
            ...
            Log.itemWarn("Components", namespacedID, "'map_color' value '{}' contains invalid hex digits.", raw);
            return false;
        }
    }
