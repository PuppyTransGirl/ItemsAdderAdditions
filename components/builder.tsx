...
const ACTIONS: Record<string, FieldDef[]> = {
    ...
    replace_biome: [
        ...
        {
            k: 'blocks_from_center',
            l: 'blocks_from_center',
            t: 'number',
            r: false,
            p: '5',
        },
    ...
    toast: [
        {
            ...
            t: 'text',
            r: true,
            p: 'minecraft:diamond or namespace:item',
        },
        {
            k: 'text',
            l: 'text (one per line)',
...
const inputCls =
    'w-full rounded-md border border-fd-border bg-fd-background px-2 py-1.5 text-sm text-fd-foreground focus:outline-none focus:ring-1 focus:ring-fd-ring';
        ...
        <div className="flex flex-col gap-1">
            <label className="flex items-center gap-1.5 text-xs text-fd-muted-foreground">
                ...
                {def.r ? (
                    ...
                    <span className="rounded px-1 py-0.5 text-[10px] bg-fd-muted text-fd-muted-foreground">
                        optional
                    </span>
                )}
            </label>
    ...
    return (
        <div className="flex flex-col gap-3">
            <Field
                ...
                value={s.sound_name}
                onChange={v => onChange('sound_name', v)}
            />
            <Field
                def={{
                    k: 'sound_volume',
                ...
                onChange={v => onChange('sound_volume', v)}
            />
            <Field
                def={{
                    k: 'sound_pitch',
                ...
                value={s.sound_pitch}
                onChange={v => onChange('sound_pitch', v)}
            />
            <Field
                def={{
                    k: 'sound_category',
                ...
                value={s.sound_category}
                onChange={v => onChange('sound_category', v)}
            />
        </div>
    );
}

function PotionEffectFields({
                                pe,
                                index,
                                onChange,
                                onRemove,
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-fd-border p-3">
            <div className="flex items-center justify-between">
                ...
                {index > 0 && (
                    <button
                        onClick={onRemove}
                        className="text-xs text-fd-muted-foreground hover:text-red-500 transition-colors"
                    >
                        Remove
                    </button>
                )}
            ...
            <Field
                def={{
                    ...
                    r: false,
                    p: '0',
                    h: '0 = level 1 (default: 0)',
                }}
                value={pe.amplifier}
                onChange={v => onChange('amplifier', v)}
            />
            <Field
                def={{
                    k: 'duration',
    ...
    return (
        <div className="flex flex-col gap-3 rounded-lg border border-fd-border p-3">
            ...
            <Field
                ...
                value={step.decrement_amount}
                onChange={v => onChange('decrement_amount', v)}
            />
...
function generateAction(type: string, values: Record<string, string>): string {
    ...
    if (type === 'replace_biome') {
        ...
        if (radiusMode === 'blocks_from_center') {
            if (blocksFromCenter) {
                lines.push(`${ind}  blocks_from_center: ${blocksFromCenter}`);
            }
        } else {
    ...
    if (target) {
        ...
        if (target === 'radius' && radius) {
            lines.push(`${ind}target_radius: ${radius}`);
        }
        if (target === 'in_sight' && inSightDistance) {
            lines.push(`${ind}target_in_sight_distance: ${inSightDistance}`);
        }
    }
    ...
    if (type === 'stackable') {
        ...
        if (mode === 'simple') {
            ...
            stackSteps.forEach((step, idx) => {
                ...
                if (step.decrement_amount.trim()) {
                    lines.push(`${i3}decrement_amount: ${step.decrement_amount.trim()}`);
                }
            });
        }
    }
...
export function Builder() {
    ...
    const setVal = useCallback(
        (k: string, v: string) => setValues(prev => ({...prev, [k]: v})),
        [],
    );
    const setExtra = useCallback(
        (k: string, v: boolean) => setExtras(prev => ({...prev, [k]: v})),
        [],
    );
                ...
                <div className="flex flex-col gap-4 rounded-xl border border-fd-border bg-fd-card p-4">
                        ...
                        <>
                            ...
                            <div className="flex flex-col gap-3">
                                {actionType === 'replace_biome' ? (
                                    <>
                                        <Field
                                            def={ACTIONS.replace_biome[0]}
                                            value={values['biome'] ?? ''}
                                            onChange={v => setVal('biome', v)}
                                        />
                                        <Field
                                            def={ACTIONS.replace_biome[1]}
                                            value={values['shape'] ?? ''}
                                            onChange={v => setVal('shape', v)}
                                        />
                                        <Field
                                            def={ACTIONS.replace_biome[2]}
                                        ...
                                        {replaceBiomeRadiusMode === 'blocks_from_center' ? (
                                            ...
                                            <>
                                                <Field
                                                    def={ACTIONS.replace_biome[4]}
                                                    value={values['radius_x'] ?? ''}
                                                    onChange={v => setVal('radius_x', v)}
                                                />
                                                <Field
                                                    def={ACTIONS.replace_biome[5]}
                                                    value={values['radius_y'] ?? ''}
                                                    onChange={v => setVal('radius_y', v)}
                                                />
                                                <Field
                                                    def={ACTIONS.replace_biome[6]}
                                                    value={values['radius_z'] ?? ''}
                                        ...
                                        <p className="text-[11px] text-fd-muted-foreground">
                                            Note: Minecraft biome editing is not perfectly exact at
                                            small scales. Small shapes may look a bit randomized or
                                            rough.
                                        </p>
                                    </>
                                ) : (
                            ...
                            <div className="flex flex-col gap-3">
                                <Field
                                    ...
                                    onChange={v => setVal('permission', v)}
                                />
                                <Field
                                    def={{
                                        k: 'delay',
                                ...
                                <Field
                                    ...
                                    value={values['u_target'] ?? ''}
                                    onChange={v => setVal('u_target', v)}
                                />
                                ...
                                {values['u_target'] === 'in_sight' && (
                                    <Field
                                        ...
                                        onChange={v =>
                                            setVal('target_in_sight_distance', v)
                                        }
                                    />
                                )}
                            </div>
                    ...
                    {mode === 'behaviour' && behaviourType === 'contact_damage' && (
                        <>
                            ...
                            <div className="flex flex-col gap-3">
                                <Field
                                    def={{k: 'amount', l: 'amount', t: 'number', r: true, p: '1.0'}}
                                    value={values['amount'] ?? ''}
                                    onChange={v => setVal('amount', v)}
                                />
                                <Field
                                    def={{
                                        k: 'interval',
                                ...
                                <Field
                                    ...
                                    value={values['fire_duration'] ?? ''}
                                    onChange={v => setVal('fire_duration', v)}
                                />
                                <Field
                                    def={{
                            ...
                            <Divider />
                            <CheckToggle
                                label="Configure block_faces"
                                checked={!!extras['faces']}
                            ...
                            <Divider />
                            <CheckToggle
                                label="Configure potion effects"
                                checked={!!extras['potions']}
                                onChange={v => {
                                    setExtra('potions', v);
                                    if (v && potions.length === 0) {
                                        setPotions([emptyPotion()]);
                                    }
                                }}
                            />
                            {extras['potions'] && (
                                <div className="flex flex-col gap-3">
                                    ...
                                    <button
                                        onClick={() =>
                                            setPotions(prev => [...prev, emptyPotion()])
                                        }
                        ...
                        <>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <Field
                                    ...
                                    value={values['open_variant'] ?? ''}
                                    onChange={v => setVal('open_variant', v)}
                                />
                            </div>
                            <Divider />
                            <CheckToggle
                                label="Add open_sound"
                                checked={!!extras['open_sound']}
                            ...
                            <Divider />
                            <CheckToggle
                                label="Add close_sound"
                                checked={!!extras['close_sound']}
                    ...
                    {mode === 'behaviour' && behaviourType === 'stackable' && (
                        <>
                            ...
                            <Field
                                ...
                                value={stackMode}
                                onChange={v => setVal('stackable_mode', v)}
                            />
                            <Divider />
                                    ...
                                    {extras['stack_sound'] && (
                                        <SubSection>
                                            <SoundFields
                                                sound={{
                                                    sound_name:
                                                        values['stack_sound_name'] ?? '',
                                                    sound_volume:
                                                        values['stack_sound_volume'] ?? '',
                                                    sound_pitch:
                                                        values['stack_sound_pitch'] ?? '',
                                                    sound_category:
                                                        values['stack_sound_category'] ?? '',
                                                }}
                                                onChange={(k, v) =>
                                                    setVal(
                                                        `stack_sound_${k.replace(
                                                            'sound_',
                                                            '',
                                                        )}`,
                                                        v,
                    ...
                    {mode === 'behaviour' && behaviourType === 'connectable' && (
                        <>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <p className="text-[11px] text-fd-muted-foreground">
                                    All variant IDs are optional. If omitted:
                                    <br/>
                                    - <code>default</code> uses the furniture this
                                    behaviour is on
                                    <br/>- every other type uses{' '}
                                    <code>{'<furniture_name>_<type>'}</code>
                                </p>
                                ...
                                {connKeys.map(k => (
                                    <Field
                                        ...
                                        def={{
                                            ...
                                            p:
                                                k === 'default'
                                                    ? 'namespace:base_furniture'
                                                    : `namespace:furniture_${k}`,
                                            h:
                                                k === 'default'
                                                    ? 'defaults to the current furniture ID'
                                                    : `defaults to <furniture_name>_${k}`,
                                        }}
