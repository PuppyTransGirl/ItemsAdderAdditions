...
/**
 * Immutable configuration snapshot for a single furniture world-populator entry.
 *
 * <p>Mirrors the fields ItemsAdder uses in {@code WorldGeneratorConfig}, but
...
public final class FurniturePopulatorConfig {
    ...
    /**
     * Worlds this populator is active in. Empty = all worlds.
     */
    public final List<String> worlds;

    /**
     * Furniture pieces per vein attempt.
     */
    public final int veinBlocks;
    ...
    /**
     * Surface-block whitelist (block directly below the spawn point). Empty =
     * any.
     */
    private final List<Material> replaceableBlocks = new ArrayList<>();

    public FurniturePopulatorConfig(
            String name,
            String filePath,
            List<String> worlds,
            String furnitureId,
        ...
        this.veinBlocks = Math.max(veinBlocks, 1);
        this.chunkVeins = Math.max(chunkVeins, 1);
    ...
    public boolean isAllowedSurface(@Nullable Material material) {
        return replaceableBlocks.isEmpty()
                || replaceableBlocks.contains(material);
    }
    ...
    void addReplaceableBlock(Material material) {
