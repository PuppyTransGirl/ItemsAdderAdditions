---
description: Automatically populate the vanilla creative menu with all your custom items *(requires 1.21.11+)*
---
...
{% hint style="warn" %}
**Version Requirement:** Requires **Paper 1.21.11 or higher**. The feature is silently disabled on older versions.
{% endhint %}
...
{% hint style="danger" %}
**Important:** To see and access the ItemsAdder items inside the creative inventory, you **must have access to the Operator Utilities tab**. This requires you to have operator permissions on the server and have the "Operator Items Tab" toggled **ON** in your Minecraft client's control settings.
{% endhint %}

## Setup

The plugin handles everything automatically on the first run. You just need to complete two quick steps after installing:

1. Run `/iazip` to regenerate and push the resource pack.
2. **Reconnect** to the server (or have players reconnect). The creative menu only updates when the client joins, not during an in-game reload.
