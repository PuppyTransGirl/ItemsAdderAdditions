...
const zh: BuilderTranslations = {
    ...
    sectionUniversalParameters: '通用参数',
    sectionFormat: '格式',
    sectionSteps: '步骤',
    sectionMode: '模式',
    sectionDisplay: '显示',
    sectionDisplays: '显示列表',
    sectionRecipeInfo: '配方信息',
    sectionRecipeType: '配方类型',
    sectionCraftingGrid: '合成网格',
    sectionIngredients: '原料',
    sectionResult: '结果',
    sectionEntryInfo: '条目信息',
    sectionFilters: '过滤器',
    sectionPlacement: '放置',
    sectionGeneratedYaml: '生成的 YAML',
    toggleAddSound: '添加声音',
    toggleAdvancedOptions: '高级选项',
    toggleConfigureBlockFaces: '配置 block_faces',
    toggleConfigurePotionEffects: '配置药水效果',
    toggleAddOpenSound: '添加 open_sound',
    toggleAddCloseSound: '添加 close_sound',
    labelSkillName: '技能名称',
    labelAnimationName: '动画名称',
    labelEmoteName: '表情名称',
    labelRadiusFormat: '半径格式',
    labelTextOneLine: '文本（每行一个）',
    labelItemsOneLine: '物品（每行一个）',
    labelBlockIdsOneLine: '方块 ID（每行一个）',
    labelSlotsOneLine: '槽位（每行一个）',
    labelDisplayId: '显示 ID',
    labelIngredientItem: '原料物品',
    labelResultItem: '结果物品',
    labelResultAmount: '结果数量',
    labelRecipeId: '配方 ID',
    labelEntryId: '条目 ID',
    labelWorldsOneLine: '世界（每行一个）',
    labelBiomesOneLine: '生物群系（每行一个）',
    labelIngredientN: (n) => `原料 ${n}`,
    noteReplaceBiome:
        '注意：Minecraft 的生物群系编辑在小范围内并非完全精确。小形状可能看起来有些随机或粗糙。',
    noteConnectable:
        '所有变体 ID 均为可选。如果省略：`default` 使用当前家具所在的行为，其他类型则使用 <家具名称>_<类型>。',
    noteCraftingGrid:
        '在每个单元格中输入一个字母。相同字母的单元格使用同一种原料。空格将在配方中变为 X。X 不能用作原料标识符。',
    noteIngredientEmpty: '未定义物品的字符将被视为空槽位。',
    tabAdvancements: '进度',
    sectionCriteria: '条件',
    sectionRewards: '奖励',
    sectionOnComplete: '完成时',
    labelAdvancementId: '进度 ID',
    labelCriterionId: '条件 ID',
    labelCommandId: '命令 ID',
    addCriterion: '+ 添加条件',
    addCommand: '+ 添加命令',
};

export const builderTranslations: Record<string, BuilderTranslations> = {en, fr, nl, zh};
