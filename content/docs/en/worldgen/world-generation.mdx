...
description: Spawn custom ItemsAdder furniture during Minecraft chunk generation. Supports world populator and surface decorator modes, configured in your content YML files.
version: "1.0.8"
---
...
| `worlds` | list | *(all)* | World names to restrict to. Supports wildcard patterns such as `world_*`. |
...
Legacy aliases are still accepted: `worlds_populators` for `blocks_populators`, `amount` for `vein_blocks`, `iterations` for `chunk_veins`, and `chance` for `chunk_chance`.
...
The legacy alias `chance` is accepted for `chunk_chance`.
...
- Surface decorators in the Nether scan downward from `max_height` to `min_height`.
- When `allow_liquid_placement` is enabled, normal-dimension surface decoration uses the ocean-floor heightmap so underwater surfaces can be found.
