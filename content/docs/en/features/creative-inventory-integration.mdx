...
description: Automatically populate the Minecraft creative inventory with all your ItemsAdder custom items. Requires Paper 1.21.5+.
version: "1.0.0"
---
...
- **Directional variants** are excluded. Items whose namespaced ID ends in `_north`, `_south`, `_east`, `_west`, `_up`, or `_down` are skipped when the base item (without the direction suffix) exists in the registry. This prevents duplicate entries for rotatable blocks.
