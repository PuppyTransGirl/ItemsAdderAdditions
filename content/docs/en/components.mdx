...
title: Components
icon: Package
description: Item data component support in ItemsAdderAdditions. Apply Minecraft data components to custom items at load time.
---

ItemsAdderAdditions applies Minecraft data components to custom items at load time via the ItemsAdder item modifier hook. Add a `components` section under your item definition and run `/iareload`.
...
Components are applied once per reload. Every item stack built after a reload carries the configured components.
...
Sets the initial contents of a bundle item. Accepts vanilla IDs, ItemsAdder IDs, and `mmoitems:TYPE:ID` format. See [compatibility](/en/docs/compatibility) for full format details.
...
Sets the projectiles pre-loaded in a crossbow. Accepts vanilla IDs, ItemsAdder IDs, and `mmoitems:TYPE:ID` format.
...
Accepts vanilla IDs, ItemsAdder IDs, and `mmoitems:TYPE:ID` format.
