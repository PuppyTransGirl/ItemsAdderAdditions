...
## Advancement region trigger

ItemsAdderAdditions 1.0.12 adds the `enter_region` advancement trigger. It awards a criterion when a player moves or teleports from outside to inside the configured WorldGuard region.
...
Region IDs are matched case-insensitively. The optional `world` value is the Bukkit world name. This trigger only needs WorldGuard to be installed; `worldguard.enabled` and `worldguard.flags.*` control protection checks, not region detection for advancements.

See the full [`enter_region` trigger reference](/en/docs/advancements/triggers#enter_region).
