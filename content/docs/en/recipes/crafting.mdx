...
Optional. Available since 1.0.13 for damageable result items.

Sets the result's **remaining durability**, not its damage value. It must be between `0` and the item's maximum
durability. This also respects a custom maximum durability applied through result components.
