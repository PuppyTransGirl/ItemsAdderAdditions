...
icon: Image
description: Register custom painting variants from your ItemsAdder YML files. Requires Paper 1.21.5+.
version: "1.0.8"
---
...
| `item` | `namespace:item_id` | *(none)* | Optional linked ItemsAdder item. When right-clicked on a valid wall face, this item places the custom painting. See [IDs](/en/docs/compatibility/ids). |
...
- WorldGuard painting placement flags are respected when WorldGuard integration is enabled.
...
- This feature is only available on Minecraft **1.21.5** and newer. On older supported versions, the plugin logs a warning and skips the `paintings` section.
