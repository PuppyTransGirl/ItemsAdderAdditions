...
| `oversized_in_gui` | Boolean | `false` | Allow the model to render outside its slot bounds in GUIs. Requires Minecraft 1.21.6 or newer. |
...
Set `oversized_in_gui: true` to allow the item model to render outside its slot bounds in inventory GUIs. Minecraft added this item model definition field in 1.21.6, so it only has an effect on 1.21.6 or newer.
...
The value is written alongside `model` at the generated JSON root. If omitted, the vanilla default of `false` applies. When used with `raw`, this top-level setting overrides `raw.oversized_in_gui`.
