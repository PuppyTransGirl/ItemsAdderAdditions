...
description: Add player-state requirements to custom advancement criteria.
version: "1.0.9"
---
...
| `location.structure` / `structures` | string or list | Match a vanilla structure key or a Custom Structures structure. Any listed structure can match. |

Vanilla structures use registry keys such as `minecraft:village_plains`. Custom Structures entries use `customstructures:<structure_name>` (the alias `custom_structures:<structure_name>` is also accepted) and require ItemsAdderAdditions 1.0.12 or higher with the Custom Structures plugin installed. See [Custom Structures compatibility](/en/docs/compatibility/customstructures).
...
Item predicates can be used in `equipment`, `slots`, and nested entity predicates. They accept ItemsAdder IDs, MMOItems IDs, vanilla item IDs, and vanilla item tags such as `#minecraft:planks`. See [IDs](/en/docs/compatibility/ids).
