---
title: Triggers
description: Reference for all built-in trigger types available for custom ItemsAdder advancements, including conditions and example YAML for each.
version: "1.0.9"
---

Triggers define what the player must do to complete a criterion. Each criterion has one trigger and an optional `conditions` block.

```yaml
criteria:
  <criterion_id>:
    trigger: <trigger_type>
    conditions:
      # trigger-specific options
```

An advancement can have multiple criteria. All of them must be completed before the advancement is awarded.

---

## obtain_item

Fires when the player picks up an item from the ground.

```yaml
criteria:
  get_sword:
    trigger: obtain_item
    conditions:
      items:
        - my_pack:ruby_sword
        - my_pack:iron_sword
      amount: 1
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `items` | list of strings | `[]` | Item IDs (IA, MMOItems, or vanilla). Any match triggers the criterion. |
| `amount` | integer | `1` | Minimum stack size the player must pick up at once. |

---

## consume_item

Fires when the player finishes eating or drinking an item.

```yaml
criteria:
  eat_ruby_apple:
    trigger: consume_item
    conditions:
      item: my_pack:ruby_apple
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `item` | string | yes | Item ID (IA, MMOItems, or vanilla). |

---

## using_item

Fires when the player right-clicks (in air or on a block) while holding a custom item in their main hand.

```yaml
criteria:
  use_staff:
    trigger: using_item
    conditions:
      item: my_pack:ruby_staff
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `item` | string | yes | Item ID (IA, MMOItems, or vanilla). |

---

## place_block

Fires when the player places a custom block registered by ItemsAdder.

```yaml
criteria:
  place_ruby_ore:
    trigger: place_block
    conditions:
      block: my_pack:ruby_ore
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `block` | string | yes | ItemsAdder block ID (`namespace:id`). |

---

## break_block

Fires when the player breaks a custom block registered by ItemsAdder.

```yaml
criteria:
  mine_ruby_ore:
    trigger: break_block
    conditions:
      block: my_pack:ruby_ore
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `block` | string | yes | ItemsAdder block ID (`namespace:id`). |

---

## place_furniture

Fires when the player places a custom furniture item.

```yaml
criteria:
  place_lamp:
    trigger: place_furniture
    conditions:
      furniture: my_pack:ruby_lamp
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `furniture` | string | yes | ItemsAdder furniture ID (`namespace:id`). |

---

## break_furniture

Fires when the player breaks a custom furniture item.

```yaml
criteria:
  break_lamp:
    trigger: break_furniture
    conditions:
      furniture: my_pack:ruby_lamp
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `furniture` | string | yes | ItemsAdder furniture ID (`namespace:id`). |

---

## interact_furniture

Fires when the player right-clicks a custom furniture item.

```yaml
criteria:
  use_workbench:
    trigger: interact_furniture
    conditions:
      furniture: my_pack:ruby_workbench
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `furniture` | string | yes | ItemsAdder furniture ID (`namespace:id`). |

---

## craft_recipe

Fires when the player crafts an item using a specific IAA recipe.

```yaml
criteria:
  craft_sword:
    trigger: craft_recipe
    conditions:
      recipe: my_pack:ruby_sword_recipe
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `recipe` | string | yes | Recipe key as `namespace:id` or a bare `id` (uses the file namespace). |

---

## kill_entity_with_item

Fires when the player kills an entity while holding a specific item in their main hand.

```yaml
criteria:
  slay_zombie:
    trigger: kill_entity_with_item
    conditions:
      item: my_pack:ruby_sword
      entity_type: minecraft:zombie  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(required)* | Item ID (IA, MMOItems, or vanilla) the player must be holding. |
| `entity_type` | string | *(any)* | Vanilla entity type key (e.g. `minecraft:zombie`). If omitted, any entity matches. |

---

## player_hurt_entity

Fires when the player deals damage to an entity. Unlike `kill_entity_with_item`, this fires on every hit, not just the killing blow.

```yaml
criteria:
  hit_zombie:
    trigger: player_hurt_entity
    conditions:
      item: my_pack:ruby_sword  # optional
      entity_type: minecraft:zombie  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID the player must be holding. If omitted, any item (or bare hands) matches. |
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any entity matches. |

---

## entity_hurt_player

Fires when an entity deals damage to the player.

```yaml
criteria:
  hurt_by_zombie:
    trigger: entity_hurt_player
    conditions:
      entity_type: minecraft:zombie  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any attacker matches. |

---

## player_killed_entity

Fires when the player kills an entity (regardless of the weapon used).

```yaml
criteria:
  kill_creeper:
    trigger: player_killed_entity
    conditions:
      entity_type: minecraft:creeper  # optional
      item: my_pack:ruby_sword  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any entity matches. |
| `item` | string | *(any)* | Item ID the player must be holding. If omitted, any item matches. |

---

## entity_killed_player

Fires when the player is killed by a non-player entity (projectile kills resolve to the shooter).

```yaml
criteria:
  killed_by_skeleton:
    trigger: entity_killed_player
    conditions:
      entity_type: minecraft:skeleton  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key of the killer. If omitted, any entity matches. |

---

## killed_by_arrow

Fires when the player is killed by an arrow. The entity type matches the arrow's shooter, or the arrow itself if it had no shooter.

```yaml
criteria:
  shot_by_skeleton:
    trigger: killed_by_arrow
    conditions:
      entity_type: minecraft:skeleton  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key of the shooter. If omitted, any arrow source matches. |

---

## tame_animal

Fires when the player tames an animal.

```yaml
criteria:
  tame_wolf:
    trigger: tame_animal
    conditions:
      entity_type: minecraft:wolf  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any tameable animal matches. |

---

## bred_animals

Fires when the player breeds two animals.

```yaml
criteria:
  breed_cows:
    trigger: bred_animals
    conditions:
      entity_type: minecraft:cow   # optional, type of the offspring
      parent_type: minecraft:cow   # optional
      partner_type: minecraft:cow  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Entity type of the offspring. |
| `parent_type` | string | *(any)* | Entity type of the mother. |
| `partner_type` | string | *(any)* | Entity type of the father. |

---

## villager_trade

Fires when the player completes a trade with a villager.

```yaml
criteria:
  buy_diamond:
    trigger: villager_trade
    conditions:
      item: minecraft:diamond  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID of the trade result. If omitted, any trade matches. |

---

## enchanted_item

Fires when the player enchants an item at an enchanting table.

```yaml
criteria:
  enchant_ruby_sword:
    trigger: enchanted_item
    conditions:
      item: my_pack:ruby_sword  # optional
      min_levels: 10            # optional
      max_levels: 30            # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID. If omitted, any item being enchanted matches. |
| `min_levels` | integer | `0` | Minimum XP level cost of the enchantment. |
| `max_levels` | integer | *(unlimited)* | Maximum XP level cost of the enchantment. |

---

## shoot_bow

Fires when the player shoots a projectile from a bow.

```yaml
criteria:
  shoot_ruby_bow:
    trigger: shoot_bow
    conditions:
      item: my_pack:ruby_bow  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID of the bow. If omitted, any bow matches. |

---

## shot_crossbow

Fires when the player shoots a projectile from a crossbow.

```yaml
criteria:
  shoot_crossbow:
    trigger: shot_crossbow
    conditions:
      item: my_pack:ruby_crossbow  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID of the crossbow. If omitted, any crossbow matches. |

---

## fishing_rod_hooked

Fires when the player reels in a fish or hooks an entity.

```yaml
criteria:
  hook_fish:
    trigger: fishing_rod_hooked
    conditions:
      rod: my_pack:ruby_rod          # optional
      caught_entity_type: minecraft:cod  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `rod` | string | *(any)* | Item ID of the fishing rod. If omitted, any rod matches. |
| `caught_entity_type` | string | *(any)* | Entity type of what was hooked. If omitted, any catch matches. |

---

## filled_bucket

Fires when the player fills a bucket with a liquid.

```yaml
criteria:
  fill_lava:
    trigger: filled_bucket
    conditions:
      item: minecraft:lava_bucket  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID of the resulting bucket. If omitted, any filled bucket matches. |

---

## item_durability_changed

Fires when an item in the player's hand takes durability damage.

```yaml
criteria:
  damage_ruby_sword:
    trigger: item_durability_changed
    conditions:
      item: my_pack:ruby_sword  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID of the damaged item. If omitted, any item matches. |

---

## item_used_on_block

Fires when the player right-clicks a block with an item in their main hand.

```yaml
criteria:
  use_on_crafting_table:
    trigger: item_used_on_block
    conditions:
      item: my_pack:ruby_wand          # optional
      block: minecraft:crafting_table  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID the player must be holding. |
| `block` | string | *(any)* | Vanilla block type key. |

---

## recipe_crafted

Fires when the player crafts any item using the vanilla crafting system.

```yaml
criteria:
  craft_diamond_sword:
    trigger: recipe_crafted
    conditions:
      recipe: minecraft:diamond_sword  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `recipe` | string | *(any)* | Recipe key as `namespace:id`. If omitted, any crafting action matches. |

---

## recipe_unlocked

Fires when the player discovers a new recipe.

```yaml
criteria:
  unlock_diamond_sword:
    trigger: recipe_unlocked
    conditions:
      recipe: minecraft:diamond_sword
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `recipe` | string | yes | Recipe key as `namespace:id`. |

---

## player_interacted_with_entity

Fires when the player right-clicks an entity with their main hand.

```yaml
criteria:
  name_a_villager:
    trigger: player_interacted_with_entity
    conditions:
      entity_type: minecraft:villager  # optional
      item: minecraft:name_tag         # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any entity matches. |
| `item` | string | *(any)* | Item ID the player must be holding. If omitted, any item (or empty hand) matches. |

---

## player_sheared_equipment

Fires when the player shears an entity.

```yaml
criteria:
  shear_sheep:
    trigger: player_sheared_equipment
    conditions:
      entity_type: minecraft:sheep  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any shearable entity matches. |

---

## bee_nest_destroyed

Fires when the player breaks a beehive or bee nest.

```yaml
criteria:
  break_beehive:
    trigger: bee_nest_destroyed
    conditions:
      block: minecraft:beehive  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `block` | string | *(any)* | Block type key (`minecraft:beehive` or `minecraft:bee_nest`). If omitted, either matches. |

---

## started_riding

Fires when the player mounts an entity.

```yaml
criteria:
  ride_horse:
    trigger: started_riding
    conditions:
      entity_type: minecraft:horse  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `entity_type` | string | *(any)* | Vanilla entity type key. If omitted, any mount matches. |

---

## effects_changed

Fires when the player gains or has a potion effect changed.

```yaml
criteria:
  get_speed:
    trigger: effects_changed
    conditions:
      effect: minecraft:speed  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `effect` | string | *(any)* | Potion effect key (e.g. `minecraft:speed`). If omitted, any effect addition or change matches. |

---

## changed_dimension

Fires when the player moves to a different dimension.

```yaml
criteria:
  enter_nether:
    trigger: changed_dimension
    conditions:
      to: minecraft:the_nether    # optional
      from: minecraft:overworld   # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `to` | string | *(any)* | Dimension key the player traveled to (e.g. `minecraft:the_nether`). |
| `from` | string | *(any)* | Dimension key the player traveled from. |

---

## slept_in_bed

Fires when the player successfully enters a bed. No conditions.

```yaml
criteria:
  first_sleep:
    trigger: slept_in_bed
```

---

## used_totem

Fires when the player is saved from death by a totem of undying. No conditions.

```yaml
criteria:
  totem_pop:
    trigger: used_totem
```

---

## fall_from_height

Fires when the player takes fall damage. No conditions.

```yaml
criteria:
  survived_fall:
    trigger: fall_from_height
```

---

## used_ender_eye

Fires when the player throws an eye of ender. No conditions.

```yaml
criteria:
  use_ender_eye:
    trigger: used_ender_eye
```

---

## held_item

IAA custom trigger. Fires when the player scrolls to a hotbar slot that contains a specific item.

```yaml
criteria:
  equip_ruby_sword:
    trigger: held_item
    conditions:
      item: my_pack:ruby_sword  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `item` | string | *(any)* | Item ID. If omitted, any item scrolled to matches. |

---

## permission

Fires when the player joins while holding the specified permission node.

```yaml
criteria:
  has_vip:
    trigger: permission
    conditions:
      permission: "myserver.rank.vip"
```

| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `permission` | string | yes | Permission node. Checked on every join; fires as soon as the player has it. |

---

## in_biome

Fires when the player crosses into a specific biome. Checked at chunk boundaries.

```yaml
criteria:
  visit_jungle:
    trigger: in_biome
    conditions:
      biome: minecraft:jungle
      world: world  # optional
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `biome` | string | *(required)* | Biome key as `namespace:id` (e.g. `minecraft:deep_dark`). |
| `world` | string | *(any)* | World name to restrict the check to a single world. |

---

## impossible

Never fires on its own. The criterion can only be granted via the IAA API or admin commands.

```yaml
criteria:
  manual:
    trigger: impossible
```

No `conditions` options.
