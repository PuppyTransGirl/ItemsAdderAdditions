---
title: Triggers
description: Reference all built-in custom advancement trigger types.
version: "1.0.9"
---
...
An advancement can have multiple criteria. All of them must be completed before the advancement is awarded. Every runtime trigger also supports an optional `conditions.player` predicate for extra player-state requirements; see [Player predicates](/en/docs/advancements/player-predicates). Item conditions support ItemsAdder IDs, MMOItems IDs, vanilla item IDs, vanilla item tags, and supported ItemsAdderAdditions custom item tags. Block, furniture, and recipe conditions support custom tags where the matching path accepts tags. See [IDs](/en/docs/compatibility/ids) for the shared syntax.
...
Fires when the player crosses into a specific biome.
...
Fires when the player moves or teleports from outside to inside a WorldGuard region.

<Callout type="warn">
    **Version requirements:** Requires **ItemsAdderAdditions 1.0.12 or higher** and WorldGuard.
</Callout>
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `region` | string | *(required)* | WorldGuard region ID. Matching is case-insensitive. |
| `world` | string | *(any)* | Bukkit world name to restrict the check to a single world. |

The criterion only fires when the player crosses the region boundary. Moving between blocks while already inside the region does not fire it again. This trigger is independent of the ItemsAdderAdditions WorldGuard flag toggles.

See [WorldGuard compatibility](/en/docs/compatibility/worldguard) for setup and troubleshooting.
