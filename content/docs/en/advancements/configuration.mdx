...
description: YAML reference for custom advancement display, criteria, and rewards.
version: "1.0.9"
---
...
| `criteria` | section | *(none)* | Conditions that must be met to complete the advancement. Each criterion has one trigger, trigger-specific conditions, and an optional `conditions.player` predicate. See [Triggers](/en/docs/advancements/triggers) and [Player predicates](/en/docs/advancements/player-predicates). |
| `rewards` | section | *(none)* | Experience, loot tables, and recipe unlocks. See [Rewards](#rewards). |
| `on_complete` | section | *(none)* | Actions run when the advancement is fully completed. See [Completion actions](/en/docs/advancements/completion-actions). |
...
| `icon` | `namespace:id` | `minecraft:barrier` | Item shown as the icon. Accepts ItemsAdder item IDs, MMOItems IDs, or vanilla material IDs. Tags are not supported because the icon must resolve to one concrete item. See [IDs](/en/docs/compatibility/ids). |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `trigger` | string | `impossible` | Trigger type for this criterion. See the full [trigger reference](/en/docs/advancements/triggers). |
| `conditions` | section | *(none)* | Trigger-specific options, plus optional `player` predicates. |
| `conditions.player` | section or list | *(any player)* | Extra player predicate checked when the criterion is about to be awarded. Ignored by the `impossible` trigger. See [Player predicates](/en/docs/advancements/player-predicates). |
...
<Callout>
    ...
    tracked independently; progress is saved by vanilla. Since ItemsAdderAdditions 1.0.12, `/iareload` keeps existing
    registrations and player progress when the advancement definitions are unchanged. Changing a definition can require
    that advancement to be re-registered.
</Callout>
