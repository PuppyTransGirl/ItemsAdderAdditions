...
| `criteria` | section | *(none)* | Conditions that must be met to complete the advancement. Each criterion has one trigger, trigger-specific conditions, and an optional `conditions.player` predicate. See [Triggers](/docs/en/advancements/triggers) and [Player predicates](/docs/en/advancements/player-predicates). |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `trigger` | string | `impossible` | Trigger type for this criterion. See the full [trigger reference](/docs/en/advancements/triggers). |
| `conditions` | section | *(none)* | Trigger-specific options, plus optional `player` predicates. |
| `conditions.player` | section or list | *(any player)* | Extra player predicate checked when the criterion is about to be awarded. Ignored by the `impossible` trigger. See [Player predicates](/docs/en/advancements/player-predicates). |
