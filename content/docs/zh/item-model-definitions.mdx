...
| `oversized_in_gui` | 布尔 | `false` | 允许模型在 GUI 中渲染到槽位边界之外。需要 Minecraft 1.21.6 或更高版本。 |
...
设置 `oversized_in_gui: true` 可允许物品模型在物品栏 GUI 中渲染到槽位边界之外。Minecraft 1.21.6 新增了此物品模型定义字段，因此该设置仅在 1.21.6 或更高版本中生效。
...
item_model_definition:
  ...
  model:
    type: model
    model: my_pack:item/large_item
```

该值会与 `model` 一同写入生成的 JSON 根对象。若省略，则使用原版默认值 `false`。与 `raw` 一起使用时，此顶层设置会覆盖 `raw.oversized_in_gui`。
