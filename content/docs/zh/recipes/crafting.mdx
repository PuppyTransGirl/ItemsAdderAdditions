...
新文件建议使用 `iaa_crafting_table`。`iaa_crafting` 行为相同，并作为别名存在。
...
共享 ID 参考请见 [ID](/zh/docs/compatibility/ids)。
...
原料还可以额外使用原版标签，如 `#minecraft:planks`。这些仅是 Bukkit 原版物品/方块标签；合成原料不会解析 ItemsAdderAdditions 自定义标签。标签不能用作结果或替换物品。
...
- 每个 `pattern...` 键都会作为 shaped pattern 变体加载。例如：`pattern`、`pattern_2`、`pattern_alt`。
- 匿名 shapeless 原料列表最多支持 26 个条目。
