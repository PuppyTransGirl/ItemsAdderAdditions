...
方块标签语法（如 `#minecraft:logs`）会根据原版方块标签进行解析。标签仅匹配原版方块；自定义方块请使用显式的 ItemsAdder 方块 ID

## 物品和装备谓词

物品谓词可用于 `equipment`、`slots` 和嵌套实体谓词。它们接受 ItemsAdder ID、MMOItems ID、原版物品 ID 和原版物品标签，如 `#minecraft:planks`
...
原版物品标签根据原版物品注册表进行解析。物品 `predicates` 部分仍然不受支持，并且会使物品谓词匹配失败，而不是意外匹配
