...
| `location.structure` / `structures` | 字符串或列表 | 匹配原版结构或 Custom Structures 结构；列表中任意一个结构匹配即可。 |

原版结构使用 `minecraft:village_plains` 等注册表键。自定义结构使用 `customstructures:<structure_name>`（也接受 `custom_structures:<structure_name>` 别名），并需要 ItemsAdderAdditions 1.0.12 或更高版本以及 Custom Structures 插件。请见 [Custom Structures 兼容性](/zh/docs/compatibility/customstructures)。
