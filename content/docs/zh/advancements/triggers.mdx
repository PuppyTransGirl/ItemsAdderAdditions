...
当玩家通过移动或传送从外部进入 WorldGuard 区域时触发。

<Callout type="warn">
    **版本要求：** 需要 **ItemsAdderAdditions 1.0.12 或更高版本**以及 WorldGuard。
</Callout>
...
| 键 | 类型 | 默认值 | 描述 |
|----|------|--------|------|
| `region` | 字符串 | *（必填）* | WorldGuard 区域 ID，不区分大小写。 |
| `world` | 字符串 | *（任意）* | 将检查限制在单个 Bukkit 世界名称。 |

该准则只在玩家跨过区域边界时触发。玩家已在区域内时继续移动不会再次触发。此触发器不受 ItemsAdderAdditions WorldGuard flag 开关影响。

安装与故障排除请见 [WorldGuard 兼容性](/zh/docs/compatibility/worldguard)。
