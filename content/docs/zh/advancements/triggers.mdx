---
title: 触发器
description: 适用于自定义 ItemsAdder 进度的所有内置触发器类型参考，包含每个触发器的准则和 YAML 示例
version: "1.0.9"
---
...
一个进度可以有多个准则。所有准则都必须完成，进度才会被授予。每个运行时触发器还支持可选的 `conditions.player` 谓词，用于额外的玩家状态要求；参见[玩家谓词](/zh/docs/advancements/player-predicates)。物品准则支持 ItemsAdder ID、MMOItems ID、原版物品 ID 和原版物品标签。方块准则支持 ItemsAdder 方块 ID、原版方块 ID 和原版方块标签
...
| 键           | 类型  | 必填 | 描述                          |
|-------------|-----|----|-----------------------------|
| `furniture` | 字符串 | 是  | ItemsAdder 家具 ID（`命名空间:id`） |
...
| 键        | 类型  | 必填 | 描述                                  |
|----------|-----|----|-------------------------------------|
| `recipe` | 字符串 | 是  | 配方键，格式为 `命名空间:id` 或纯 `id`（使用文件命名空间） |
...
| 键        | 类型  | 默认值    | 描述                               |
|----------|-----|--------|----------------------------------|
| `recipe` | 字符串 | *（任意）* | 配方键，格式为 `命名空间:id`。如果省略，任意合成操作均匹配 |
...
| 键        | 类型  | 默认值    | 描述                                 |
|----------|-----|--------|------------------------------------|
| `recipe` | 字符串 | *（任意）* | 配方键，格式为 `命名空间:id`。如果省略，任意已发现的配方均匹配 |
...
| 键       | 类型  | 默认值    | 描述                                            |
|---------|-----|--------|-----------------------------------------------|
| `biome` | 字符串 | *（必填）* | 生物群系键，格式为 `命名空间:id`（例如 `minecraft:deep_dark`） |
...
自身永远不会触发。该准则只能通过 IAA API 或管理员命令授予
