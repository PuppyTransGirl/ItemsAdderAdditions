...
Définit le contenu initial d'un objet bourse. Accepte les IDs vanilla, les IDs ItemsAdder, et le format `mmoitems:TYPE:ID`. Voir [compatibilité](/fr/docs/compatibility) pour les détails complets du format.
...
Définit les projectiles préchargés dans une arbalète. Accepte les IDs vanilla, les IDs ItemsAdder, et le format `mmoitems:TYPE:ID`.
...
Accepte les IDs vanilla, les IDs ItemsAdder, et le format `mmoitems:TYPE:ID`.
