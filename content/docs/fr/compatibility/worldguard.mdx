...
| `iaa_replace_near_blocks` | `iaa-action-iaa-replace-near-blocks` |
...
iaa-action-iaa-replace-near-blocks
...
## Déclencheur d'avancement de région

ItemsAdderAdditions 1.0.12 ajoute le déclencheur `enter_region`. Il valide un critère lorsqu'un joueur se déplace ou se téléporte de l'extérieur vers l'intérieur d'une région WorldGuard configurée.
...
La casse des IDs de région est ignorée. La valeur optionnelle `world` est le nom du monde Bukkit. Ce déclencheur nécessite seulement WorldGuard ; `worldguard.enabled` et `worldguard.flags.*` contrôlent les protections, pas la détection des régions pour les avancements.

Consultez la [référence du déclencheur `enter_region`](/fr/docs/advancements/triggers#enter_region).
...
- [Déclencheurs d'avancement](/fr/docs/advancements/triggers)
