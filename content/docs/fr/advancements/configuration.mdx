...
| `criteria` | section | *(aucun)* | Conditions qui doivent être remplies pour terminer le progrès. Voir [Déclencheurs](/docs/fr/advancements/triggers). |
| `rewards` | section | *(aucun)* | Expérience et déverrouillage de recettes. Voir [Récompenses](#récompenses). |
| `on_complete` | section | *(aucun)* | Actions exécutées lorsque le progrès est entièrement terminé. Voir [Actions d’achèvement](/docs/fr/advancements/completion-actions). |
...
| `background` | chaîne | *(aucun)* | **Progrès racines uniquement.** Chemin de texture pour l’arrière-plan de l’onglet (par exemple `minecraft:textures/gui/advancements/backgrounds/adventure.png`). |
