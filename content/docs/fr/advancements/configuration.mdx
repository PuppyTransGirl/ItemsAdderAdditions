...
| `criteria` | section | *(aucun)* | Conditions qui doivent être remplies pour terminer le progrès. Chaque critère possède un déclencheur, des conditions propres au déclencheur et un prédicat `conditions.player` optionnel. Voir [Déclencheurs](/docs/advancements/triggers) et [Prédicats de joueur](/docs/advancements/player-predicates). |
...
| Clé | Type | Par défaut | Description |
|-----|------|---------|-------------|
| `trigger` | chaîne | `impossible` | Type de déclencheur pour ce critère. Voir la [référence complète des déclencheurs](/docs/advancements/triggers). |
| `conditions` | section | *(aucun)* | Options propres au déclencheur, plus les prédicats `player` optionnels. |
| `conditions.player` | section ou liste | *(n’importe quel joueur)* | Prédicat de joueur supplémentaire vérifié au moment où le critère est sur le point d’être attribué. Ignoré par le déclencheur `impossible`. Voir [Prédicats de joueur](/docs/advancements/player-predicates). |
