...
| `location.structure` / `structures` | chaîne ou liste | Correspond à une structure vanilla ou à une structure Custom Structures. N'importe quelle entrée peut correspondre. |

Les structures vanilla utilisent des clés de registre comme `minecraft:village_plains`. Les structures personnalisées utilisent `customstructures:<nom_de_structure>` (l'alias `custom_structures:<nom_de_structure>` est aussi accepté) et nécessitent ItemsAdderAdditions 1.0.12 ou supérieur avec le plugin Custom Structures. Consultez la [compatibilité Custom Structures](/fr/docs/compatibility/customstructures).
