...
Un progrès peut avoir plusieurs critères. Tous doivent être terminés avant que le progrès soit accordé. Chaque déclencheur d’exécution prend aussi en charge un prédicat `conditions.player` optionnel pour ajouter des exigences liées à l’état du joueur ; voir [Prédicats de joueur](/fr/docs/advancements/player-predicates). Les conditions d’objets acceptent les IDs ItemsAdder, MMOItems, vanilla et les tags d’objets vanilla. Les conditions de blocs acceptent les IDs de blocs ItemsAdder, les IDs de blocs vanilla et les tags de blocs vanilla.
