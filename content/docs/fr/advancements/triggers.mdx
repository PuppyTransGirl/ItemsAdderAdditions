---
title: Déclencheurs
description: Référence des déclencheurs de progrès ItemsAdder personnalisés, avec conditions et exemples YAML.
version: "1.0.10"
---
...
Un progrès peut avoir plusieurs critères. Tous doivent être terminés avant que le progrès soit accordé. Chaque déclencheur d’exécution prend aussi en charge un prédicat `conditions.player` optionnel pour ajouter des exigences liées à l’état du joueur ; voir [Prédicats de joueur](/fr/docs/advancements/player-predicates). Les conditions d’objets acceptent les IDs ItemsAdder, MMOItems, vanilla, les tags vanilla et les tags personnalisés pris en charge. Les conditions de blocs, meubles et recettes peuvent aussi utiliser les tags personnalisés lorsque le déclencheur les accepte.
...
|-----|------|---------|-------------|
| `items` | liste de chaînes | `[]` | IDs d’objets ou tags d’objets vanilla (IA, MMOItems, vanilla ou `#minecraft:tag`). Toute correspondance déclenche le critère. |
...
|-----|------|----------|-------------|
| `item` | chaîne | Oui | ID d’objet ou tag d’objet vanilla (IA, MMOItems, vanilla ou `#minecraft:tag`). |
...
| Clé | Type | Requis | Description |
|-----|------|----------|-------------|
| `block` | chaîne | oui | ID de bloc ItemsAdder, ID de bloc vanilla ou tag de bloc vanilla comme `#minecraft:logs`. |
...
| Clé | Type | Par défaut | Description |
|-----|------|---------|-------------|
| `item` | chaîne | *(requis)* | ID d’objet ou tag d’objet vanilla que le joueur doit tenir. |
...
|-----|------|--------|-------------|
| `distance` | number ou section | `0` à illimité | Plage de distance de chute. Un nombre indique une distance minimale. Une section peut utiliser `min`/`max`, ou une plage imbriquée `y`, `absolute` ou `horizontal`. |
| `distance.min` | number | `0` | Distance de chute minimale. |
| `distance.max` | number | *(illimité)* | Distance de chute maximale. |
| `distance.y` | section | *(aucun)* | Forme recommandée pour la plage verticale de distance de chute. Prend en charge `min` et `max`. |
| `distance.absolute` | section | *(aucun)* | Forme de plage alternative. Prend en charge `min` et `max`. |
| `distance.horizontal` | section | *(aucun)* | Forme de plage alternative. Prend en charge `min` et `max`. |
...
Se déclenche lorsque le joueur entre dans un biome spécifique.
...
Se déclenche lorsqu'un joueur se déplace ou se téléporte de l'extérieur vers l'intérieur d'une région WorldGuard.

<Callout type="warn">
    **Versions requises :** Nécessite **ItemsAdderAdditions 1.0.12 ou supérieur** et WorldGuard.
</Callout>
...
| Clé | Type | Par défaut | Description |
|-----|------|------------|-------------|
| `region` | chaîne | *(requis)* | ID de région WorldGuard. La casse est ignorée. |
| `world` | chaîne | *(n'importe quel)* | Nom du monde Bukkit pour limiter la vérification à un seul monde. |

Le critère ne se déclenche qu'au franchissement de la limite de la région. Se déplacer à l'intérieur de celle-ci ne le déclenche pas à nouveau. Ce déclencheur est indépendant des options de flags WorldGuard d'ItemsAdderAdditions.

Consultez la [compatibilité WorldGuard](/fr/docs/compatibility/worldguard) pour l'installation et le dépannage.
...
| Clé | Type | Requis | Description |
|-----|------|--------|-------------|
| `furniture` | chaîne | oui | ID de meuble ItemsAdder ou tag personnalisé de meuble pris en charge. |
...
| Clé | Type | Requis | Description |
|-----|------|--------|-------------|
| `trade_machine` | chaîne | oui | ID de machine d'échange ou tag personnalisé de meuble pris en charge. |
...
| Clé | Type | Par défaut | Description |
|-----|------|------------|-------------|
| `emote` | chaîne | *(n'importe laquelle)* | ID d'emote. Si absent, n'importe quelle emote correspond. |
...
| Clé | Type | Par défaut | Description |
|-----|------|------------|-------------|
| `emote` | chaîne | *(n'importe laquelle)* | ID d'emote. Si absent, n'importe quel arrêt d'emote correspond. |
