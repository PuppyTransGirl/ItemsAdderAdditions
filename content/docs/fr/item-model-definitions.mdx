...
| `oversized_in_gui` | Booléen | `false` | Autorise le modèle à dépasser les limites de son emplacement dans les interfaces. Nécessite Minecraft 1.21.6 ou plus récent. |
...
Définissez `oversized_in_gui: true` pour autoriser le modèle de l'objet à dépasser les limites de son emplacement dans les interfaces d'inventaire. Minecraft a ajouté ce champ de définition de modèle d'objet dans la version 1.21.6 ; il n'a donc d'effet qu'à partir de cette version.
...
La valeur est écrite à côté de `model` à la racine du JSON généré. Si elle est omise, la valeur vanilla par défaut `false` s'applique. Avec `raw`, ce paramètre de premier niveau remplace `raw.oversized_in_gui`.
