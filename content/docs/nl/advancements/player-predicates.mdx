...
| `location.structure` / `structures` | string of lijst | Komt overeen met een vanilla structure of een Custom Structures-structure. Elke vermelding kan matchen. |

Vanilla structures gebruiken registry keys zoals `minecraft:village_plains`. Custom Structures-vermeldingen gebruiken `customstructures:<structure_name>` (de alias `custom_structures:<structure_name>` wordt ook geaccepteerd) en vereisen ItemsAdderAdditions 1.0.12 of hoger met de Custom Structures-plugin. Zie [Custom Structures-compatibiliteit](/nl/docs/compatibility/customstructures).
