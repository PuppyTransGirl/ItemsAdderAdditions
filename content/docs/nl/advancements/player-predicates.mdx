...
Bloktagsyntaxis zoals `#minecraft:logs` wordt opgelost met vanilla-bloktags. Tags komen alleen overeen met vanilla-blokken; gebruik expliciete ItemsAdder-blok-ID's voor aangepaste blokken.

## Item- en uitrustingspredicaten

Itempredicaten kunnen worden gebruikt in `equipment`, `slots` en geneste entiteitspredicaten. Ze accepteren ItemsAdder-ID's, MMOItems-ID's, vanilla item-ID's en vanilla itemtags zoals `#minecraft:planks`.
...
Vanilla itemtags worden opgelost met het vanilla-itemregister. Itemsecties `predicates` worden nog niet ondersteund en laten het itempredicaat gesloten falen in plaats van per ongeluk te matchen.
