...
| `oversized_in_gui` | Boolean | `false` | Staat toe dat het model buiten de grenzen van het slot in GUI's wordt gerenderd. Vereist Minecraft 1.21.6 of nieuwer. |
...
Stel `oversized_in_gui: true` in om toe te staan dat het itemmodel buiten de grenzen van het slot in inventaris-GUI's wordt gerenderd. Minecraft heeft dit veld voor item model definitions toegevoegd in 1.21.6, dus het heeft alleen effect op 1.21.6 of nieuwer.
...
De waarde wordt naast `model` in de root van de gegenereerde JSON geschreven. Wanneer deze is weggelaten, geldt de vanilla standaardwaarde `false`. Bij gebruik met `raw` overschrijft deze instelling op het hoogste niveau `raw.oversized_in_gui`.
