...
## Regiotrigger voor advancements

ItemsAdderAdditions 1.0.12 voegt de `enter_region`-trigger toe. Deze voltooit een criterium wanneer een speler van buiten naar binnen een ingestelde WorldGuard-regio loopt of teleporteert.
...
Hoofdletters in regio-ID's worden genegeerd. De optionele waarde `world` is de Bukkit-wereldnaam. Deze trigger heeft alleen WorldGuard nodig; `worldguard.enabled` en `worldguard.flags.*` beheren beschermingscontroles, niet regiodetectie voor advancements.

Zie de [`enter_region`-triggerreferentie](/nl/docs/advancements/triggers#enter_region).
...
- [Advancementtriggers](/nl/docs/advancements/triggers)
